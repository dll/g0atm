...
class test_DbUtil(unittest.TestCase):
    ...
    def test_validate_pin_true(self):
        ...
        cpin = "123456";
        self.assertEqual(self.dbutil.validate_pin(cpin, DbUtil.input_pin.return_value), 1, "密码匹配")

    def test_validate_pin_false(self):
        DbUtil.input_pin = Mock()
        DbUtil.input_pin.return_value = "123456"
        cpin = "222222";
        self.assertEqual(self.dbutil.validate_pin(cpin, DbUtil.input_pin.return_value), 0, "密码错误")

    def test_validate_pin_less6(self):
        DbUtil.input_pin = Mock()
        DbUtil.input_pin.return_value = "23456"
        cpin = "222222";
        self.assertEqual(self.dbutil.validate_pin(cpin, DbUtil.input_pin.return_value), 0, "密码小于6位数字")

    def test_validate_pin_more6(self):
        DbUtil.input_pin = Mock()
        DbUtil.input_pin.return_value = "1234567"
        cpin = "222222";
        self.assertEqual(self.dbutil.validate_pin(cpin, DbUtil.input_pin.return_value), 0, "密码大于6位数字")